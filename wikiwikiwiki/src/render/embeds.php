...
function render_embed_google(string $value, string $original): string
{
    $term = trim($value);
    if ($term === '') {
        return render_embed_invalid($original);
    }
    $url = 'https://www.google.com/search?q=' . rawurlencode($term) . '&hl=ko&gl=KR';
    return '<a href="' . html($url) . '" class="external google" target="_blank" rel="noopener noreferrer">' . html($term) . '</a>';
}
